...
async def save_stakeholders(
    ...
    existing_stakeholders = (
        ...
        .filter(
            Stakeholder.project_id == project_id, Stakeholder.substep_id == substep_id
        )
        .all()
    )
    ...
    for name in existing_names - frontend_stakeholders:
        db.query(Stakeholder).filter(
            ...
            Stakeholder.substep_id == substep_id,
...
async def create_project(
    ...
    if not template_id:
        template = (
            db.query(ProjectTemplate).filter(ProjectTemplate.is_default == True).first()
        )
        if not template:
    ...
    template_steps = (
        db.query(TemplateStep)
        .filter(TemplateStep.template_id == template.id)
    ...
    for t_step in template_steps:
        ...
        template_substeps = (
            db.query(TemplateSubstep)
            .filter(TemplateSubstep.template_step_id == t_step.id)
        ...
        for t_substep in template_substeps:
            ...
            template_subtasks = (
                db.query(TemplateSubtask)
                .filter(TemplateSubtask.template_substep_id == t_substep.id)
...
async def get_projects(
    ...
    all_projects = list(all_projects) + [
        p for p in member_projects if p.id not in [cp.id for cp in created_projects]
    ]
...
async def get_project_detail(
    ...
    steps = (
        ...
        .filter(ProjectStep.project_id == project_id)
    ...
    for step in steps:
        substeps = (
            db.query(ProjectSubstep)
            .filter(ProjectSubstep.project_step_id == step.id)
        ...
        for substep in substeps:
            subtasks = (
                db.query(ProjectSubtask)
                .filter(ProjectSubtask.project_substep_id == substep.id)
...
async def save_substep_content(
    ...
    db_content = (
        db.query(SubstepContent)
        .filter(SubstepContent.project_substep_id == substep.id)
        .first()
    )
    ...
    await notify_content_saved(
        ...
        content_data={
            "substep_id": substep_id,
            "updated_at": (
                db_content.updated_at.isoformat() if db_content.updated_at else None
            ),
            "user_id": current_user.id,
        },
    )
...
async def get_project_stakeholders(
    ...
    stakeholders = (
        db.query(Stakeholder).filter(Stakeholder.project_id == project_id).all()
    )
