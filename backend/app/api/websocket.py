...
async def user_websocket_endpoint(websocket: WebSocket, token: str = Query(None)):
    ...
    if token:
        try:
            ...
            logger.error(f"User WS Token validation failed: {e}")
    ...
    connected = await manager.connect_user(websocket, user_id)
    if not connected:
        await websocket.close(code=1011, reason="Connection failed")
        return

    print(f"[WS USER] CONNECTED: User={username}(ID:{user_id})", flush=True)
...
async def websocket_endpoint(
    ...
    print(
        f"[WS] CONNECTED: User={username}(ID:{user_id}) to Project={project_id}",
        flush=True,
    )
